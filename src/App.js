...
function App() {
  ...
  const [view, setView] = useState('flashcards'); // 'flashcards' or 'studyPlan'
        ...
        <div className="flex-1 p-8">
          <div className="max-w-2xl mx-auto">
            {view === 'flashcards' ? (
              ...
              <StudyPlan onProblemClick={(pattern) => {
                const relatedCard = flashcards.find(card => card.title === pattern);
                if (relatedCard) {
                  setView('flashcards');
                  setCategory(relatedCard.category);
                  setFilter('All');
                  // Find the index of the card in the filtered list
                  const cardIndex = flashcards.findIndex(card => card.title === pattern);
                  if (cardIndex !== -1) {
